...
public class ListManager : MonoBehaviour
{
    ...
    private List<LeaderboardEntryUI> listItems = new List<LeaderboardEntryUI>();
    ...
    private void InitializeList()
    {
        ...
        Container.sizeDelta = Direction == LISTDIRECTION.HORIZONTAL
            ? (new Vector2(prefabSize * Num, prefabScale.y))
            : (new Vector2(prefabScale.x, prefabSize * Num));
        containerHalfSize = Direction == LISTDIRECTION.HORIZONTAL
            ? (Container.rect.size.x * 0.5f)
            : (Container.rect.size.y * 0.5f);

        numVisible = Mathf.CeilToInt((Direction == LISTDIRECTION.HORIZONTAL ? maskRT.rect.size.x : maskRT.rect.size.y) /
                                     prefabSize);

        offsetVec = Direction == LISTDIRECTION.HORIZONTAL ? Vector3.right : Vector3.down;
        startPos = Container.anchoredPosition3D - (offsetVec * containerHalfSize) +
        ...
        for (int i = 0; i < numItems; i++)
        {
            ...
            LeaderboardEntryUI li = obj.GetComponentInChildren<LeaderboardEntryUI>();
            listItems.Add(li);
            //li.UpdateContent(i);
        }

        Prefab.gameObject.SetActive(false);
        Container.anchoredPosition3D += offsetVec * (containerHalfSize -
                                                     ((Direction == LISTDIRECTION.HORIZONTAL
                                                         ? maskRT.rect.size.x
                                                         : maskRT.rect.size.y) * 0.5f));
    }
